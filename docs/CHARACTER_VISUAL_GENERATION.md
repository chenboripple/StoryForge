...
- 当前支持 `main`、`gallery`、`video` 三类角色视觉资产。
- 主形象、艺术照、视频立体图均通过任务队列异步执行，生成结果落盘到本地并通过本地 URL 暴露。
- 封面与角色主图的 prompt draft 已接入多 Agent 协作链路。

## 后续计划

1. 继续完善图生图能力，减少对纯文本特征抽取的依赖。
2. 为视觉任务补充更细的失败原因展示和重试策略。
