"""
StoryForge - 重构后的创作 Agent V3
改进点：
1. 使用 PromptAssembler 动态组装 prompt
2. 支持大纲细化阶段
3. 融入人味化规则
4. 结构化 Review/Proofread 输出
"""
...
class ReviewerAgentV3(BaseAgent):
    """青锋 - 审稿节点 V3（结构化输出 + AI味评估）"""
...
class ReviserAgentV3(BaseAgent):
    """修改节点 V3"""
...
class ProofreaderAgentV3(BaseAgent):
    """砚清 - 校对节点 V3（结构化输出 + 终审）"""
